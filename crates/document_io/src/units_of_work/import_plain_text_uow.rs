...
use crate::use_cases::import_plain_text_uc::{
    ImportPlainTextUnitOfWorkFactoryTrait, ImportPlainTextUnitOfWorkTrait,
};
...
// Unit of work for ImportPlainText

pub struct ImportPlainTextUnitOfWork {
    context: DbContext,
...
impl ImportPlainTextUnitOfWork {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        ImportPlainTextUnitOfWork {
...
impl CommandUnitOfWork for ImportPlainTextUnitOfWork {
...
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/import_plain_text_uc.rs
//
...
pub struct ImportPlainTextUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl ImportPlainTextUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        ImportPlainTextUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl ImportPlainTextUnitOfWorkFactoryTrait for ImportPlainTextUnitOfWorkFactory {
    fn create(&self) -> Box<dyn ImportPlainTextUnitOfWorkTrait> {
        Box::new(ImportPlainTextUnitOfWork::new(
            &self.context,
            &self.event_hub,
        ))
    }
