...
use crate::use_cases::replace_text_uc::{
    ReplaceTextUnitOfWorkFactoryTrait, ReplaceTextUnitOfWorkTrait,
};
...
// Unit of work for ReplaceText

pub struct ReplaceTextUnitOfWork {
    context: DbContext,
...
impl ReplaceTextUnitOfWork {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        ReplaceTextUnitOfWork {
...
impl CommandUnitOfWork for ReplaceTextUnitOfWork {
...
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/replace_text_uc.rs
//
...
impl ReplaceTextUnitOfWorkTrait for ReplaceTextUnitOfWork {}

pub struct ReplaceTextUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl ReplaceTextUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        ReplaceTextUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl ReplaceTextUnitOfWorkFactoryTrait for ReplaceTextUnitOfWorkFactory {
    fn create(&self) -> Box<dyn ReplaceTextUnitOfWorkTrait> {
        Box::new(ReplaceTextUnitOfWork::new(&self.context, &self.event_hub))
    }
}
