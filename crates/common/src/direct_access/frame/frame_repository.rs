...
pub struct FrameRepository<'a> {
    redb_table: Box<dyn FrameTable + 'a>,
...
impl<'a> FrameRepository<'a> {
    ...
    pub fn snapshot(&self, ids: &[EntityId]) -> Result<EntityTreeSnapshot, RepositoryError> {
        ...
        {
            // Extract child IDs from the forward junction snapshot for blocks
            let junction_name = "block_from_frame_blocks_junction";
    ...
    pub fn restore(
        ...
        for child_snap in &snap.children {
            if child_snap.table_data.entity_rows.table_name == "block" {
                repository_factory::write::create_block_repository(self.transaction)
...
pub struct FrameRepositoryRO<'a> {
    redb_table: Box<dyn FrameTableRO + 'a>,
}
impl<'a> FrameRepositoryRO<'a> {
    pub fn new(redb_table: Box<dyn FrameTableRO + 'a>) -> Self {
        FrameRepositoryRO { redb_table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Frame>, RepositoryError> {
        self.redb_table.get(id)
    }
