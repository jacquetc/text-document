...
const RESOURCE_TABLE: TableDefinition<EntityId, Postcard<Resource>> =
    TableDefinition::new("resource");
...
pub struct ResourceRedbTable<'a> {
    transaction: &'a WriteTransaction,
}
impl<'a> ResourceRedbTable<'a> {
    pub fn new(transaction: &'a WriteTransaction) -> Self {
        ResourceRedbTable { transaction }
    }

    pub fn init_tables(transaction: &WriteTransaction) -> Result<(), RepositoryError> {
        transaction.open_table(RESOURCE_TABLE)?;
...
impl<'a> ResourceTable for ResourceRedbTable<'a> {
    ...
    fn create_multi(&mut self, entities: &[Resource]) -> Result<Vec<Resource>, RepositoryError> {
        ...
        let mut counter = if let Some(counter) = counter_table.get(&"resource".to_string())? {
            counter.value()
        } else {
            1
        };
        let mut resource_table = self.transaction.open_table(RESOURCE_TABLE)?;
        ...
        for entity in entities {
            let new_entity = if entity.id == EntityId::default() {
                ...
                if resource_table.get(&entity.id)?.is_some() {
            ...
            resource_table.insert(new_entity.id, new_entity.clone())?;
        ...
        counter_table.insert("resource".to_string(), counter)?;
        Ok(created)
    }
    ...
    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Resource>>, RepositoryError> {
        ...
        let resource_table = self.transaction.open_table(RESOURCE_TABLE)?;

        for id in ids {
            let item = if let Some(guard) = resource_table.get(id)? {
    ...
    fn get_all(&self) -> Result<Vec<Resource>, RepositoryError> {
        ...
        let mut iter = resource_table.iter()?;
    ...
    fn update_multi(&mut self, entities: &[Resource]) -> Result<Vec<Resource>, RepositoryError> {
        ...
        for entity in entities {
            resource_table.insert(entity.id, entity)?;
        }
        drop(resource_table);
    ...
    fn remove_multi(&mut self, ids: &[EntityId]) -> Result<(), RepositoryError> {
        ...
        let mut backward_document_resources_junction_table = self
            .transaction
        ...
        for id in ids {
            ...
            db_helpers::delete_from_backward_junction_table(
                &mut backward_document_resources_junction_table,
                id,
            )?;
        }
        Ok(())
    ...
    fn snapshot_rows(&self, ids: &[EntityId]) -> Result<TableLevelSnapshot, RepositoryError> {
        ...
        for id in ids {
            if let Some(guard) = resource_table.get(id)? {
    ...
    fn restore_rows(&mut self, snap: &TableLevelSnapshot) -> Result<(), RepositoryError> {
        ...
        for (id, bytes) in &snap.entity_rows.rows {
            let entity: Resource = postcard::from_bytes(bytes)
                .map_err(|e| RepositoryError::Serialization(e.to_string()))?;
            resource_table.insert(*id, entity)?;
        }
...
pub struct ResourceRedbTableRO<'a> {
    transaction: &'a ReadTransaction,
}
impl<'a> ResourceRedbTableRO<'a> {
    pub fn new(transaction: &'a ReadTransaction) -> Self {
        ResourceRedbTableRO { transaction }
    }
}

impl<'a> ResourceTableRO for ResourceRedbTableRO<'a> {
    fn get(&self, id: &EntityId) -> Result<Option<Resource>, RepositoryError> {
        let v = self.get_multi(std::slice::from_ref(id))?;
        Ok(v.into_iter().next().unwrap())
    }
