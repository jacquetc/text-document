...
use crate::use_cases::insert_markdown_at_position_uc::{
    InsertMarkdownAtPositionUnitOfWorkFactoryTrait, InsertMarkdownAtPositionUnitOfWorkTrait,
};
...
// Unit of work for InsertMarkdownAtPosition

pub struct InsertMarkdownAtPositionUnitOfWork {
    context: DbContext,
...
impl InsertMarkdownAtPositionUnitOfWork {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        InsertMarkdownAtPositionUnitOfWork {
...
impl CommandUnitOfWork for InsertMarkdownAtPositionUnitOfWork {
...
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/insert_markdown_at_position_uc.rs
//
...
impl InsertMarkdownAtPositionUnitOfWorkTrait for InsertMarkdownAtPositionUnitOfWork {}

pub struct InsertMarkdownAtPositionUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl InsertMarkdownAtPositionUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        InsertMarkdownAtPositionUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl InsertMarkdownAtPositionUnitOfWorkFactoryTrait for InsertMarkdownAtPositionUnitOfWorkFactory {
    fn create(&self) -> Box<dyn InsertMarkdownAtPositionUnitOfWorkTrait> {
        Box::new(InsertMarkdownAtPositionUnitOfWork::new(
            &self.context,
            &self.event_hub,
        ))
    }
