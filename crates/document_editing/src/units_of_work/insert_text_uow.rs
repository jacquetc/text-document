use crate::use_cases::insert_text_uc::{
    InsertTextUnitOfWorkFactoryTrait, InsertTextUnitOfWorkTrait,
};
...
// Unit of work for InsertText

pub struct InsertTextUnitOfWork {
    context: DbContext,
...
impl InsertTextUnitOfWork {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        InsertTextUnitOfWork {
...
impl CommandUnitOfWork for InsertTextUnitOfWork {
...
impl InsertTextUnitOfWorkTrait for InsertTextUnitOfWork {}

pub struct InsertTextUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl InsertTextUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        InsertTextUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl InsertTextUnitOfWorkFactoryTrait for InsertTextUnitOfWorkFactory {
    fn create(&self) -> Box<dyn InsertTextUnitOfWorkTrait> {
        Box::new(InsertTextUnitOfWork::new(&self.context, &self.event_hub))
    }
}
