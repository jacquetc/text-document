...
use crate::use_cases::insert_frame_uc::{
    InsertFrameUnitOfWorkFactoryTrait, InsertFrameUnitOfWorkTrait,
};
...
// Unit of work for InsertFrame

pub struct InsertFrameUnitOfWork {
    context: DbContext,
...
impl InsertFrameUnitOfWork {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        InsertFrameUnitOfWork {
...
impl CommandUnitOfWork for InsertFrameUnitOfWork {
...
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/insert_frame_uc.rs
//
...
impl InsertFrameUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        InsertFrameUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl InsertFrameUnitOfWorkFactoryTrait for InsertFrameUnitOfWorkFactory {
    fn create(&self) -> Box<dyn InsertFrameUnitOfWorkTrait> {
        Box::new(InsertFrameUnitOfWork::new(&self.context, &self.event_hub))
    }
}
