...
use crate::use_cases::create_list_uc::{
    CreateListUnitOfWorkFactoryTrait, CreateListUnitOfWorkTrait,
};
...
// Unit of work for CreateList

pub struct CreateListUnitOfWork {
    context: DbContext,
...
impl CreateListUnitOfWork {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        CreateListUnitOfWork {
...
impl CommandUnitOfWork for CreateListUnitOfWork {
...
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/create_list_uc.rs
//
...
impl CreateListUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        CreateListUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl CreateListUnitOfWorkFactoryTrait for CreateListUnitOfWorkFactory {
    fn create(&self) -> Box<dyn CreateListUnitOfWorkTrait> {
        Box::new(CreateListUnitOfWork::new(&self.context, &self.event_hub))
    }
}
