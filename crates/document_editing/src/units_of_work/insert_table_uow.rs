use crate::use_cases::insert_table_uc::{
    InsertTableUnitOfWorkFactoryTrait, InsertTableUnitOfWorkTrait,
};
...
pub struct InsertTableUnitOfWork {
    context: DbContext,
...
impl InsertTableUnitOfWork {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        InsertTableUnitOfWork {
...
impl CommandUnitOfWork for InsertTableUnitOfWork {
...
pub struct InsertTableUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl InsertTableUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        InsertTableUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl InsertTableUnitOfWorkFactoryTrait for InsertTableUnitOfWorkFactory {
    fn create(&self) -> Box<dyn InsertTableUnitOfWorkTrait> {
        Box::new(InsertTableUnitOfWork::new(&self.context, &self.event_hub))
    }
}
