use crate::use_cases::delete_text_uc::{
    DeleteTextUnitOfWorkFactoryTrait, DeleteTextUnitOfWorkTrait,
};
...
// Unit of work for DeleteText

pub struct DeleteTextUnitOfWork {
    context: DbContext,
...
impl DeleteTextUnitOfWork {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        DeleteTextUnitOfWork {
...
impl CommandUnitOfWork for DeleteTextUnitOfWork {
...
impl DeleteTextUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        DeleteTextUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl DeleteTextUnitOfWorkFactoryTrait for DeleteTextUnitOfWorkFactory {
    fn create(&self) -> Box<dyn DeleteTextUnitOfWorkTrait> {
        Box::new(DeleteTextUnitOfWork::new(&self.context, &self.event_hub))
    }
}
