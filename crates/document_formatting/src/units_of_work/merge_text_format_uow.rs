...
use crate::use_cases::merge_text_format_uc::{
    MergeTextFormatUnitOfWorkFactoryTrait, MergeTextFormatUnitOfWorkTrait,
};
...
// Unit of work for MergeTextFormat

pub struct MergeTextFormatUnitOfWork {
    context: DbContext,
...
impl MergeTextFormatUnitOfWork {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        MergeTextFormatUnitOfWork {
...
impl CommandUnitOfWork for MergeTextFormatUnitOfWork {
...
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/merge_text_format_uc.rs
//
...
impl MergeTextFormatUnitOfWorkTrait for MergeTextFormatUnitOfWork {}

pub struct MergeTextFormatUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl MergeTextFormatUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        MergeTextFormatUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl MergeTextFormatUnitOfWorkFactoryTrait for MergeTextFormatUnitOfWorkFactory {
    fn create(&self) -> Box<dyn MergeTextFormatUnitOfWorkTrait> {
        Box::new(MergeTextFormatUnitOfWork::new(
            &self.context,
            &self.event_hub,
        ))
    }
