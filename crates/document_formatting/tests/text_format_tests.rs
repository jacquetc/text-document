...
use document_formatting::{
    CharVerticalAlignment, SetTextFormatDto, UnderlineStyle,
};

use test_harness::{
    BlockRelationshipField, FrameRelationshipField, UpdateBlockDto, UpdateInlineElementDto,
    block_controller, frame_controller, get_block_ids, get_sorted_cells,
    inline_element_controller, insert_image, insert_table, create_list, setup_with_text,
};
...
fn test_set_text_format_block_with_image() -> Result<()> {
    ...
    document_formatting_controller::set_text_format(
        &db, &hub, &mut urm, None,
        &SetTextFormatDto { position: 0, anchor: 2, font_bold: Some(true), ..Default::default() },
    )?;

    let elem_ids = block_controller::get_relationship(&db, &block_id, &BlockRelationshipField::Elements)?;
    let mut found_bold_text = false;
    let mut image_is_bold = false;
    for eid in &elem_ids {
        let elem = inline_element_controller::get(&db, eid)?.unwrap();
        match &elem.content {
            InlineContent::Text(t) if t == "AB" => { assert_eq!(elem.fmt_font_bold, Some(true)); found_bold_text = true; }
            InlineContent::Image { .. } => { image_is_bold = elem.fmt_font_bold == Some(true); }
            InlineContent::Text(t) if t == "CD" => { assert_ne!(elem.fmt_font_bold, Some(true), "CD should not be bold"); }
            _ => {}
        }
    }
...
fn test_set_text_format_including_image() -> Result<()> {
    ...
    document_formatting_controller::set_text_format(
        &db, &hub, &mut urm, None,
        &SetTextFormatDto { position: 0, anchor: 5, font_italic: Some(true), ..Default::default() },
    )?;
    ...
    for eid in &elem_ids {
        let elem = inline_element_controller::get(&db, eid)?.unwrap();
        assert_eq!(elem.fmt_font_italic, Some(true), "All elements (text and image) should be italic");
    }
    Ok(())
}
...
fn test_set_text_format_unicode_split() -> Result<()> {
    ...
    document_formatting_controller::set_text_format(
        ...
        &SetTextFormatDto {
            position: 1, anchor: 3,
            font_underline: Some(true), underline_style: Some(UnderlineStyle::DashUnderline),
            ..Default::default()
        },
    )?;
    ...
    let block_ids = get_block_ids(&db)?;
    let elem_ids = block_controller::get_relationship(&db, &block_ids[0], &BlockRelationshipField::Elements)?;
    assert!(elem_ids.len() >= 3, "Unicode text should be split into at least 3 parts, got {}", elem_ids.len());
    ...
    for eid in &elem_ids {
        ...
        if elem.fmt_font_underline == Some(true) {
            if let InlineContent::Text(ref t) = elem.content {
                ...
                assert_eq!(elem.fmt_underline_style, Some(common::entities::UnderlineStyle::DashUnderline));
...
fn test_set_text_format_in_list_blocks() -> Result<()> {
    ...
    let list_result = create_list(&db, &hub, &mut urm, 0, 27, common::entities::ListStyle::Decimal)?;

    document_formatting_controller::set_text_format(
        &db, &hub, &mut urm, None,
        &SetTextFormatDto { position: 0, anchor: 13, font_bold: Some(true), font_family: Some("Serif".into()), ..Default::default() },
    )?;

    let block_ids = get_block_ids(&db)?;

    let elem_ids_0 = block_controller::get_relationship(&db, &block_ids[0], &BlockRelationshipField::Elements)?;
    ...
    let elem_ids_1 = block_controller::get_relationship(&db, &block_ids[1], &BlockRelationshipField::Elements)?;
    let mut found_bold_item = false;
    for eid in &elem_ids_1 {
        let elem = inline_element_controller::get(&db, eid)?.unwrap();
        if let InlineContent::Text(ref t) = elem.content {
            if elem.fmt_font_bold == Some(true) { assert_eq!(t, "Item"); found_bold_item = true; }
        }
    }
...
fn test_set_text_format_in_table_cell() -> Result<()> {
    ...
    let cell_block_ids = frame_controller::get_relationship(&db, &cell_frame_id, &FrameRelationshipField::Blocks)?;
    let cell_block = block_controller::get(&db, &cell_block_ids[0])?.unwrap();
    let cell_block_pos = cell_block.document_position;

    let elem_ids = block_controller::get_relationship(&db, &cell_block.id, &BlockRelationshipField::Elements)?;
    ...
    document_formatting_controller::set_text_format(
        &db, &hub, &mut urm, None,
        &SetTextFormatDto { position: cell_block_pos, anchor: cell_block_pos + 4, font_bold: Some(true), font_italic: Some(true), ..Default::default() },
    )?;

    let cell_block_ids_after = frame_controller::get_relationship(&db, &cell_frame_id, &FrameRelationshipField::Blocks)?;
    let elem_ids_after = block_controller::get_relationship(&db, &cell_block_ids_after[0], &BlockRelationshipField::Elements)?;
    let mut found_bold_cell = false;
    for eid in &elem_ids_after {
        let elem = inline_element_controller::get(&db, eid)?.unwrap();
        if elem.fmt_font_bold == Some(true) && elem.fmt_font_italic == Some(true) {
            if let InlineContent::Text(ref t) = elem.content { assert_eq!(t, "Cell"); found_bold_cell = true; }
        }
    }
    assert!(found_bold_cell, "Should find bold+italic 'Cell' in table cell");
    Ok(())
}
...
fn test_set_text_format_undo_redo_with_split() -> Result<()> {
    ...
    document_formatting_controller::set_text_format(
        &db, &hub, &mut urm, None,
        &SetTextFormatDto { position: 2, anchor: 5, font_bold: Some(true), ..Default::default() },
    )?;

    let block_ids = get_block_ids(&db)?;
    let elem_ids_after = block_controller::get_relationship(&db, &block_ids[0], &BlockRelationshipField::Elements)?;
    assert!(elem_ids_after.len() >= 3, "Should have at least 3 elements after split");

    urm.undo(None)?;
    let elem_ids_undo = block_controller::get_relationship(&db, &block_ids[0], &BlockRelationshipField::Elements)?;
    ...
    urm.redo(None)?;
    let elem_ids_redo = block_controller::get_relationship(&db, &block_ids[0], &BlockRelationshipField::Elements)?;
    assert!(elem_ids_redo.len() >= 3, "After redo should have 3+ elements again");
    let mut found_bold = false;
    for eid in &elem_ids_redo {
        let e = inline_element_controller::get(&db, eid)?.unwrap();
        if e.fmt_font_bold == Some(true) { found_bold = true; }
    }
...
fn test_set_text_format_empty_range_no_op() -> Result<()> {
    ...
    document_formatting_controller::set_text_format(
        &db, &hub, &mut urm, None,
        &SetTextFormatDto { position: 3, anchor: 3, font_bold: Some(true), ..Default::default() },
    )?;
...
fn test_set_text_format_all_fields() -> Result<()> {
    ...
    document_formatting_controller::set_text_format(
        ...
        &SetTextFormatDto {
            position: 0, anchor: 4,
            font_family: Some("Monospace".into()), font_point_size: Some(24), font_weight: Some(900),
            font_bold: Some(true), font_italic: Some(true), font_underline: Some(true),
            font_overline: Some(true), font_strikeout: Some(true),
            letter_spacing: Some(3), word_spacing: Some(6),
    ...
    assert_eq!(elem.fmt_underline_style, Some(common::entities::UnderlineStyle::WaveUnderline));
    assert_eq!(elem.fmt_vertical_alignment, Some(common::entities::CharVerticalAlignment::SuperScript));
    Ok(())
}
