...
impl CommandUnitOfWork for DocumentWriteUoW {
...
impl use_cases::WriteUoW for DocumentWriteUoW {
    type Entity = Document;

    fn get(&self, id: &EntityId) -> Result<Option<Document>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_document_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Document>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_document_repository(transaction)?;
    ...
    fn create_orphan_multi(&self, entities: &[Document]) -> Result<Vec<Document>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_document_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Document]) -> Result<Vec<Document>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_document_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Document]) -> Result<Vec<Document>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_document_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for DocumentWriteUoW {
    fn create_multi(
        &self,
        entities: &[Document],
        owner_id: EntityId,
...
impl use_cases::WriteRelUoW<DocumentRelationshipField> for DocumentWriteUoW {
    fn set_relationship(
        &self,
        id: &EntityId,
...
pub struct DocumentWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl DocumentWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        DocumentWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for DocumentWriteUoWFactory {
    type Entity = Document;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Document>> {
        Box::new(DocumentWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for DocumentWriteUoWFactory {
    type Entity = Document;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Document>> {
        Box::new(DocumentWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<DocumentRelationshipField> for DocumentWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<DocumentRelationshipField>> {
        Box::new(DocumentWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
pub struct DocumentReadUoW {
    context: DbContext,
    transaction: RefCell<Option<Transaction>>,
}

impl DocumentReadUoW {
...
impl QueryUnitOfWork for DocumentReadUoW {
...
impl use_cases::ReadUoW for DocumentReadUoW {
    type Entity = Document;

    fn get(&self, id: &EntityId) -> Result<Option<Document>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_document_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<DocumentRelationshipField> for DocumentReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &DocumentRelationshipField,
...
pub struct DocumentReadUoWFactory {
    context: DbContext,
}

impl DocumentReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        DocumentReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for DocumentReadUoWFactory {
    type Entity = Document;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Document>> {
        Box::new(DocumentReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<DocumentRelationshipField> for DocumentReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<DocumentRelationshipField>> {
        Box::new(DocumentReadUoW::new(&self.context))
    }
}
