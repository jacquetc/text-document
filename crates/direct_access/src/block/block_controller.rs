...
mod tests {
    ...
    use crate::frame::dtos::CreateFrameDto;
    use crate::frame::frame_controller;
    ...
    fn create_one(ctx: &mut TestContext) -> BlockDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateBlockDto = created.into();
