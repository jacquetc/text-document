// Generated by Qleany v1.5.1 from entity_units_of_work.tera
...
impl CommandUnitOfWork for TableCellWriteUoW {
...
impl use_cases::WriteUoW for TableCellWriteUoW {
    type Entity = TableCell;

    fn get(&self, id: &EntityId) -> Result<Option<TableCell>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_table_cell_repository(transaction);
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<TableCell>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_table_cell_repository(transaction);
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<TableCell>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_table_cell_repository(transaction);
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[TableCell]) -> Result<Vec<TableCell>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_table_cell_repository(transaction);
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[TableCell]) -> Result<Vec<TableCell>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_table_cell_repository(transaction);
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[TableCell]) -> Result<Vec<TableCell>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_table_cell_repository(transaction);
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for TableCellWriteUoW {
    fn create_multi(
        &self,
        entities: &[TableCell],
        owner_id: EntityId,
...
impl use_cases::WriteRelUoW<TableCellRelationshipField> for TableCellWriteUoW {
    fn set_relationship(
        &self,
        id: &EntityId,
...
pub struct TableCellWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl TableCellWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        TableCellWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for TableCellWriteUoWFactory {
    type Entity = TableCell;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = TableCell>> {
        Box::new(TableCellWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for TableCellWriteUoWFactory {
    type Entity = TableCell;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = TableCell>> {
        Box::new(TableCellWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<TableCellRelationshipField> for TableCellWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<TableCellRelationshipField>> {
        Box::new(TableCellWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
pub struct TableCellReadUoW {
    context: DbContext,
    transaction: RefCell<Option<Transaction>>,
}

impl TableCellReadUoW {
...
impl QueryUnitOfWork for TableCellReadUoW {
...
impl use_cases::ReadUoW for TableCellReadUoW {
    type Entity = TableCell;

    fn get(&self, id: &EntityId) -> Result<Option<TableCell>> {
        let transaction = self.transaction.borrow();
        let repo =
            repository_factory::read::create_table_cell_repository(transaction.as_ref().unwrap());
        Ok(repo.get(id)?)
    }
...
impl use_cases::ReadRelUoW<TableCellRelationshipField> for TableCellReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &TableCellRelationshipField,
...
pub struct TableCellReadUoWFactory {
    context: DbContext,
}

impl TableCellReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        TableCellReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for TableCellReadUoWFactory {
    type Entity = TableCell;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = TableCell>> {
        Box::new(TableCellReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<TableCellRelationshipField> for TableCellReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<TableCellRelationshipField>> {
        Box::new(TableCellReadUoW::new(&self.context))
    }
}
