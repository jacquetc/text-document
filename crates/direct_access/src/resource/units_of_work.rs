...
impl CommandUnitOfWork for ResourceWriteUoW {
...
impl use_cases::WriteUoW for ResourceWriteUoW {
    ...
    fn get(&self, id: &EntityId) -> Result<Option<Resource>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_resource_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Resource>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_resource_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<Resource>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_resource_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[Resource]) -> Result<Vec<Resource>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_resource_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Resource]) -> Result<Vec<Resource>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_resource_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Resource]) -> Result<Vec<Resource>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_resource_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for ResourceWriteUoW {
    fn create_multi(
        &self,
        entities: &[Resource],
        owner_id: EntityId,
...
pub struct ResourceWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl ResourceWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        ResourceWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for ResourceWriteUoWFactory {
    type Entity = Resource;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Resource>> {
        Box::new(ResourceWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for ResourceWriteUoWFactory {
    type Entity = Resource;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Resource>> {
        Box::new(ResourceWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
pub struct ResourceReadUoW {
    context: DbContext,
    transaction: RefCell<Option<Transaction>>,
}

impl ResourceReadUoW {
...
impl QueryUnitOfWork for ResourceReadUoW {
...
impl use_cases::ReadUoW for ResourceReadUoW {
    type Entity = Resource;

    fn get(&self, id: &EntityId) -> Result<Option<Resource>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_resource_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
pub struct ResourceReadUoWFactory {
    context: DbContext,
}

impl ResourceReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        ResourceReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for ResourceReadUoWFactory {
    type Entity = Resource;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Resource>> {
        Box::new(ResourceReadUoW::new(&self.context))
    }
}
