...
mod tests {
    ...
    use super::*;
    use crate::block::block_controller;
    use crate::block::dtos::CreateBlockDto;
    ...
    fn create_one(ctx: &mut TestContext) -> InlineElementDto {
        create_orphan(
            &ctx.db,
            &ctx.hub,
            &mut ctx.undo,
    ...
    fn test_update() {
        let mut ctx = TestContext::new();
        let created = create_one(&mut ctx);
        let update_dto: UpdateInlineElementDto = created.into();
